...
- 领域层 quality-gate catalog 反向依赖 infra：已把 catalog 下沉到 `domain.quality_gate_catalog`；`infra.quality_gate_catalog` 现仅作为薄转发模块存在。
...
- shipped mainline 已移除已登记 compatibility surface；任何未来兼容面都必须重新登记到 `docs/compatibility_matrix.md` 与 `robot_sim.app.compatibility_matrix`。
- live render backend 缺失时仍会退化为 placeholder / snapshot renderer，但该行为属于 render backend 自身的能力降级，不属于 compatibility surface。
- 主链不再依赖 widget/window/controller alias、scene geometry metadata 旁路或 `BootstrapContext` 的旧式解包/索引访问。
