...
- `bootstrap()` 现返回 attribute-only `BootstrapContext`；旧 tuple/indexed 解包路径已退役，主链统一改为属性访问 `context.project_root / context.container`。
- live render backend 缺失时仍会退化为 placeholder / snapshot renderer，但该行为只存在于 render backend 层；稳定 Qt 构造面本身已不再伪装为可用。

- 历史兼容旁路：bootstrap/config/main-window/worker lifecycle 的运行时兼容壳已退役；`docs/compatibility_matrix.md` 仅保留退役状态说明与门禁策略。
