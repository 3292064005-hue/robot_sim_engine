...
class TimeoutSupervisor:
    ...
    def arm(self, timeout_ms: int | None, *, task_id: str, callback: Callable[[str], None]) -> None:
        ...
        if app is not None and not bool(getattr(app, '_robot_sim_headless_helper', False)):
            thread = app.thread() if hasattr(app, 'thread') else None
            if thread is not None and hasattr(thread, 'loopLevel'):
                try:
                    loop_level = int(thread.loopLevel())
                except (TypeError, ValueError, RuntimeError):
                    loop_level = 0
        if app is not None and not bool(getattr(app, '_robot_sim_headless_helper', False)) and loop_level > 0:
