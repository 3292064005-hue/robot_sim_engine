...
try:
    ...
    from PySide6.QtWidgets import QApplication, QMessageBox, QSplitter, QTabWidget, QVBoxLayout, QWidget
...
def _safe_add_widget(layout, widget) -> None:
    """Add widgets when the target layout accepts the provided object.

    Unit tests patch GUI panels with lightweight dummy objects that do not inherit
    from ``QWidget``. Real Qt layouts reject those with ``TypeError``, so we degrade
    gracefully in test-only compatibility paths.
    """
...
def _safe_add_tab(tab_widget, widget, label: str) -> None:
    """Add tabs when the provided widget is compatible with the current backend."""
    add_tab = getattr(tab_widget, 'addTab', None)
    if callable(add_tab):
        try:
...
class MainWindowUIMixin:
    ...
    def _build_ui_without_qt_shell(self: 'MainWindowLike') -> None:
        """Construct bindable panel objects without instantiating real Qt widgets."""
    ...
    def _build_ui(self: 'MainWindowLike') -> None:
        if not _qt_app_is_running() or not isinstance(self, QWidget):
            self._build_ui_without_qt_shell()
            return
        central = QWidget()
        ...
        _safe_add_widget(v_split, self.plots_panel)
    ...
    def _build_left_column(self: 'MainWindowLike') -> QWidget:
        ...
        self.solver_panel.apply_defaults(solver_ops.solver_defaults())
        self.solver_panel.apply_trajectory_defaults(trajectory_ops.trajectory_defaults())
        _safe_add_widget(left_layout, self.robot_panel)
        _safe_add_widget(left_layout, self.target_panel)
        _safe_add_widget(left_layout, self.solver_panel)
        return left
    ...
    def _build_center_column(self: 'MainWindowLike') -> QWidget:
        ...
        self.scene_controller = SceneController(self.scene_widget)
        self.playback_panel = PlaybackPanel()
        _safe_add_widget(center_layout, self.scene_toolbar)
        _safe_add_widget(center_layout, self.scene_widget)
        _safe_add_widget(center_layout, self.playback_panel)
        return center
    ...
    def _build_right_column(self: 'MainWindowLike') -> QWidget:
        ...
        _safe_add_tab(self.right_tabs, self.status_panel, '状态')
        _safe_add_tab(self.right_tabs, self.diagnostics_panel, '诊断')
        _safe_add_tab(self.right_tabs, self.benchmark_panel, 'Benchmark')
