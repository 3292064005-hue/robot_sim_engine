...
<<<<<<< HEAD
    ...
    def remember_request(self, request) -> object:
        """Store the active IK request until terminal projection.

        Args:
            request: Immutable IK request associated with the active worker.

        Returns:
            object: The stored request for fluent call sites.

        Raises:
            None: This helper only mutates coordinator-local state.
        """
        self._pending_request = request
        return request

    def pop_pending_request(self):
        """Return and clear the active IK request, if any."""
