from __future__ import annotations

from dataclasses import dataclass, field

from robot_sim.domain.types import FloatArray


@dataclass(frozen=True)
class IKIterationLog:
    iter_idx: int
    pos_err_norm: float
    ori_err_norm: float
    cond_number: float
    manipulability: float
    dq_norm: float = 0.0
    elapsed_ms: float = 0.0
    effective_mode: str = ""
    attempt_idx: int = 0
    damping_lambda: float = 0.0
    score: float = 0.0
    step_clipped: bool = False


@dataclass(frozen=True)
class IKResult:
    """Canonical inverse-kinematics result contract."""

    success: bool
    q_sol: FloatArray
    logs: tuple[IKIterationLog, ...]
    message: str
    final_pos_err: float = float("nan")
    final_ori_err: float = float("nan")
    final_cond: float = float("nan")
    final_manipulability: float = float("nan")
    final_dq_norm: float = 0.0
    elapsed_ms: float = 0.0
    effective_mode: str = ""
    stop_reason: str = ""
    best_q: FloatArray | None = None
    restarts_used: int = 0
    diagnostics: dict[str, object] = field(default_factory=dict)
    status: str = ''
    residuals: dict[str, float] = field(default_factory=dict)
    attempt_count: int = 0
    warnings: tuple[str, ...] = ()

    def __post_init__(self) -> None:
        object.__setattr__(self, 'diagnostics', dict(self.diagnostics or {}))
        residuals = dict(self.residuals or {})
        if 'position' not in residuals and self.final_pos_err == self.final_pos_err:
            residuals['position'] = float(self.final_pos_err)
        if 'orientation' not in residuals and self.final_ori_err == self.final_ori_err:
            residuals['orientation'] = float(self.final_ori_err)
        object.__setattr__(self, 'residuals', residuals)
        resolved_status = self.status or ('succeeded' if self.success else (self.stop_reason or 'failed'))
        object.__setattr__(self, 'status', str(resolved_status))
        if self.attempt_count <= 0:
            object.__setattr__(self, 'attempt_count', max(int(self.restarts_used) + 1, 1))
        object.__setattr__(self, 'warnings', tuple(str(item) for item in self.warnings))
