...
class RenderBackendUnavailableError(RobotSimError):
    """Raised when an optional render backend is unavailable for the requested operation."""

    default_error_code = 'render_backend_unavailable'
    default_remediation_hint = '安装 GUI / 渲染依赖，或切换到不依赖该 backend 的运行模式。'


class RenderInitializationError(RobotSimError):
    """Raised when a render backend is present but fails during initialization."""

    default_error_code = 'render_initialization_failed'
    default_remediation_hint = '检查图形依赖、Qt 平台插件和驱动环境。'


class RenderOperationError(RobotSimError):
    """Raised when a render/scene operation fails after initialization."""

    default_error_code = 'render_operation_failed'
    default_remediation_hint = '检查当前渲染状态和场景对象是否仍然有效。'


class PlotBackendUnavailableError(RobotSimError):
    """Raised when the optional plotting backend is unavailable."""
