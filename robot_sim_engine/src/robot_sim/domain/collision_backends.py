...
class CollisionBackendRegistry:
    ...
    def active_backend_ids(self, *, experimental_enabled: bool = False) -> tuple[str, ...]:
        """Return backend identifiers that are currently selectable in the active runtime mode."""
        active: list[str] = []
        for descriptor in self._descriptors:
            availability = descriptor.availability(experimental_enabled=experimental_enabled)
            if availability == 'enabled':
                active.append(descriptor.backend_id)
        return tuple(active)
