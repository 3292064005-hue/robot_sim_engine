...
class MetricsService:
    ...
    def summarize_trajectory(self, trajectory: JointTrajectory) -> dict[str, float | int | bool | str]:
        ...
        return {
            ...
            'planner_label': str(canonical.get('planner_label', canonical['planner_id'])),
            'planner_family': str(canonical.get('planner_family', 'unknown')),
            'planner_status': str(canonical.get('status', 'unknown')),
            'stable_surface': bool(canonical.get('stable_surface', False)),
