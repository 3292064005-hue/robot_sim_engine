...
class ShippedLMIKSolver(LevenbergMarquardtIKSolver):
    """Repository-shipped production solver plugin used to keep the stable plugin path alive.
    ...
    def solve(self, spec, target, q0, config, cancel_flag=None, progress_cb=None, *, attempt_idx: int = 0):
        ...
        if getattr(getattr(config, 'mode', None), 'value', None) is None and str(getattr(config, 'mode', '') or '') in {'shipped_lm_solver'}:
...
def build_plugin():
    ...
    return plugin_payload(
        ShippedLMIKSolver(),
            ...
            'notes': 'Repository-shipped production solver plugin used by the default runtime surface.',
